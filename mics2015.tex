...
Clojure utilizes the Java Virtual Machine (JVM) as an interpreter, and Clojure code compiles to JVM bytecode.  which means that it compiles to JVM bytecode, and it offers
easy 
%\emcomment{accessibilty?} 
accessibility of Java frameworks. The Clojure REPL is another easy way to get acquainted with Clojure.
The REPL allows users to see immediate results and respond easily and quickly. For any programmer, the use of the REPL is highly advised and can make programming in Clojure much simpler.
...
In the above example, we define a string to be \texttt{"Hello World"}. This way, whenever we call \texttt{mystring},
...
In this example, we saved a function that takes a number and increments it by adding 1 to it. In any function, the function name is the word after the \texttt{defn}, the pieces in the square brackets after the function name indicate the arguments that the function takes in, and what follows is the body, or what the function does.

Now let's say that we have a program where we want to use a function, but only once. This means that we do not
necessarily need to define it with a name because Clojure supports anonymous functions. Anonymous functions allow programmers to quickly make
...
structures listed can contain any number, including \texttt{nil} (Clojure's equivalent of \texttt{null} or nothing), of values of any type (a data structure
is untyped, which means it does not care if types of values in one structure are mismatched):
...
A more realistic example would look something like:
\begin{verbatim}
	{:a 1, :b 2, :c 3}
\end{verbatim}
